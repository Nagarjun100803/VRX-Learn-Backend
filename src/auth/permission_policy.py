...
from typing import FrozenSet, Literal, NamedTuple, Union
...
READ_AND_UPDATE = Policy(
    "read-and-update",
    capabilities=frozenset({Action.VIEW, Action.UPDATE}),
    scope="global",
)
"""**`READ_AND_UPDATE`** allows `view` and `update` actions globally."""

OWNED_READ_AND_UPDATE = Policy(
    "owned-read-and-update",
    capabilities=frozenset({Action.VIEW, Action.UPDATE}),
    scope="contextual",
)
...
OWNED_READ_ONLY = Policy(
    "owned-read-only", capabilities=frozenset({Action.VIEW}), scope="contextual"
)
"""**`OWNED_READ_ONLY`** allows only `view` action but only on owned resources."""

CREATE_AND_VIEW = Policy(
    "create-and-view",
    capabilities=frozenset({Action.CREATE, Action.VIEW}),
    scope="global",
)
"""**`CREATE_AND_VIEW`** allows `create` and `view` actions globally but does not allow update and delete actions."""

OWNED_CREATE_AND_VIEW = Policy(
    "owned-create-and-view",
    capabilities=frozenset({Action.CREATE, Action.VIEW}),
    scope="contextual",
)
...
def get_policy(user_role: UserRole, entity: Entity) -> Policy:
    """
    ...
    if UserRole(user_role) == UserRole.ADMIN:
        return CRUD_ALL
    policy = POLICY_V1.get(user_role, {}).get(entity)
    if policy is None:
        raise ValueError(f"No policy defined for role {user_role} and entity {entity}")
    return policy
