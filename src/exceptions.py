...
class EntityNotFoundError(DomainError):
    ...
    _default: ClassVar[str] = (
        "No '{display_name}' found with the '{identifier}' = '{value}';"
    )
...
class AlreadyExistsError(DomainError):
    ...
    _entity: ClassVar[str]
    _default: ClassVar[str] = (
        "{display_name} already exists with '{identifier}' = '{value}'."
    )
...
class InvalidContentTypeError(ValidationError):
    _template = (
        "Invalid content type '{content_type}'. Allowed types are: {allowed_types}."
    )
    ...
    def __init__(
        ...
        self.allowed_types = (
            allowed_types._member_names_
            if issubclass(allowed_types, StrEnum)  # type: ignore
            else allowed_types
        )
        message = message or self._template.format(
            content_type=content_type, allowed_types=self.allowed_types
        )
...
class FileSizeExceededError(ValidationError):
    ...
    _template = (
        "The uploaded file exceeds the maximum allowed size of {max_size} bytes."
    )

    def __init__(self, max_size: Optional[int], message: Optional[str] = None) -> None:
...
class InvalidRoleError(ValidationError):
    ...
    def __init__(
        self, role: Optional["UserRole"] = None, message: Optional[str] = None
    ) -> None:
...
class MaxAttemptsReachedError(ValidationError):
    ...
    def __init__(
        self, max_attempts: Optional[int] = None, message: Optional[str] = None
    ) -> None:
...
class InvalidScoreError(ValidationError):
    ...
    def __init__(
        self, max_score: Optional[int] = None, message: Optional[str] = None
    ) -> None:
        ...
        if message is not None:
            ...
            final_messgae = (
                f"{self._default}, Score must be less than or equal to {max_score}"
            )
