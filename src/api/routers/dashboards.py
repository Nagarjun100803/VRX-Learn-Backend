...
admin_router = APIRouter(
    prefix="/dashboard/admin", tags=["Dashboard", "Admin Dashboard"]
)


@admin_router.get("/kpis", response_model=AdminKPI)
async def get_admin_kpis(
    query_service: AdminDashboardQueryServiceDependency, current_user: CurrentAdmin
):
    return await query_service.get_kpis()


@admin_router.get("/top-enrolled-courses", response_model=list[AdminCourseCard])
...
trainee_router = APIRouter(
    prefix="/dashboard/trainee", tags=["Dashboard", "Trainee Dashboard"]
)
...
@trainee_router.get("/current-course", response_model=Optional[CourseCard])
...
trainer_router = APIRouter(
    prefix="/dashboard/trainer", tags=["Dashboard", "Trainer Dashboard"]
)


@trainer_router.get("/kpis", response_model=TrainerKPI)
async def get_trainer_kpis(
    query_service: TrainerDashboardQueryServiceDependency, current_user: CurrentTrainer
):
