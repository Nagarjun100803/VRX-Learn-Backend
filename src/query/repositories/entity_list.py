...
class EntityListQueryRepository(BaseQueryRepository, PaginatorMixin):
    ...
    async def modules(self, course_id: int) -> list[ModuleDetail]:
        ...
        sql = (
            ...
            .select(module_table.id, module_table.title)
            .get_sql()
        )
        ...
        return cast(
            list[ModuleDetail], await self.db.execute(executable, fetch_returns="all")
        )
    ...
    async def lessons(self, module_id: int) -> list[LessonDetail]:
        ...
        executable = ExecutableSQL(
            sql, values=(MediableType.LESSON, MediaStatus.UPLOADED, module_id)
        )

        return cast(
            list[LessonDetail], await self.db.execute(executable, fetch_returns="all")
        )
        ...
        if filters.name:
            ...
            if filters.sort_by_enrollment_date:
                sql = sql.orderby(
                    ...
                    order=get_sort_order(filters.sort_by_enrollment_date),
                )
    ...
    async def users(
        ...
        if filters.name_or_email:
            ...
            if filters.sort_by_created_at:
                sql = sql.orderby(
                    user_table.created_at,
                    order=get_sort_order(filters.sort_by_created_at),
                )
    ...
    async def courses(
        ...
        sql = (
            PostgreSQLQuery.with_(trainee_count_query, trainee_count_cte._table_name)
            .from_(course_table)
            .join(user_table)
            ...
            sql = sql.where(
                Criterion.any(
                    terms=[
                        ...
                        course_table.title.ilike(
                            f"{filters.course_name_or_trainer_name}%"
                        ),
                        user_table.username.ilike(
                            f"{filters.course_name_or_trainer_name}%"
                        ),
                    ]
                )
    ...
    async def assignment_submissions(
        ...
        if filters.from_date:
            sql = sql.where(
                fn.Cast(assignment_submission_table.created_at, SqlTypes.DATE).between(
                    lower=filters.from_date, upper=filters.to_date
                )
            )
        else:
            sql = sql.where(
                fn.Cast(assignment_submission_table.created_at, SqlTypes.DATE)
                <= filters.to_date
            )
        ...
        if filters.sort_by_grade:
            sql = sql.orderby(
                assignment_submission_table.score,
                order=get_sort_order(filters.sort_by_grade),
            )
    ...
    async def search_users(self, username_or_email: str) -> list[UserSearchDetail]:
        ...
        sql = (
            ...
            .where(
                Criterion.any(
                    terms=[
                        user_table.username.ilike(Parameter("$1")),  # type: ignore
                        user_table.email.ilike(Parameter("$2")),  # type: ignore
                    ]
                )
            )
    ...
    @map_to_dto(dto=CourseSearchDetail, dto_mode="list")
    async def search_courses(self, course_name: str) -> list[CourseSearchDetail]:
