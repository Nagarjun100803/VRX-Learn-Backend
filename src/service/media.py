...
class MediaService:
    ...
    async def delete(self, cmd: MediaDelete, connection: Optional[Connection] = None) -> Media:
        return self._require_entity(
            await self.repo.delete(cmd, connection=connection),
            value=cmd.id
        )
    
    
    async def get(self, query: MediaGet, connection: Optional[Connection] = None) -> Media:
        return self._require_entity(
            await self.repo.get(query, connection=connection),
            value=query.id
        )
