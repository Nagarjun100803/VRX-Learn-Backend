...
from pypika import Criterion, PostgreSQLQuery, Table, functions as fn
...
class PositioningService:
    ...
    async def reorder(
        ...
        self._validate_participants(participants=participants, participants_meta=participants_meta)
        
        preceding_position = participants_meta.preceding.position_string if participants_meta.preceding else None
        succeeding_position = participants_meta.succeeding.position_string if participants_meta.succeeding else None
        ...
        new_position_string = self._get_new_position_string(preceding_position, succeeding_position)
        ...
        if updated_position is None:
            raise EntityNotFoundError(value=participants.target_id, alias="Target participant")
    ...
    async def generate_position(
        ...
        new_position_string = self._get_new_position_string(
            preceding_string=max_position_string,
            succeeding_string=None
        )
    ...
    def _get_new_position_string(
        self,
        preceding_string: Optional[str],
        succeeding_string: Optional[str]
    ) -> str:
    ...
    def _validate_target(
        ...
        if target_meta is None:
            raise EntityNotFoundError(
                message=f"Target participant not found with this ID: {target_id}"
            )
    ...
    def _validate_participants(
        ...
        self._validate_target(target_id=participants.target_id, target_meta=participants_meta.target)
        
        # NOTE: At this place the we make sure the target_meta is not None.
        self._validate_preceding(preceding_id=participants.preceding_id, preceding_meta=participants_meta.preceding, target_meta=participants_meta.target)
        
        self._validate_succeeding(succeeding_id=participants.succeeding_id, succeeding_meta=participants_meta.succeeding, target_meta=participants_meta.target)
    ...
    async def _update_position(
        ...
        sql = PostgreSQLQuery\
            ...
            .returning(table.position_string)
            
        executable = self.db.query_builder.build_executable(
            sql=sql.get_sql(), values=tuple()
        )
    ...
    async def _get_reorder_participants(
        ...
        participant_query = PostgreSQLQuery\
            .from_(table)\
            .where(
                Criterion.all(
                    terms=[
        ...
        sql = PostgreSQLQuery\
            .with_(participant_query, participant_cte._table_name)\
            .select(
                PostgreSQLQuery\
                    .from_(participant_cte)\
                    .where(participant_cte.id == participants.preceding_id)\
                    .select(
                        fn.Cast(
                            RowToJson(participant_cte), 
                            PGSqlTypes.JSONB
                        ).as_("preceding")
                    ),
        ...
        partipants_meta = await self.db.execute(executable, fetch_returns="one")
    ...
    async def _get_max_position_string(
        ...
        sql = PostgreSQLQuery\
            ...
            .select(
                fn.Max(table.position_string).as_("max_position_string")
            ).get_sql()
        ...
        return record["max_position_string"]
